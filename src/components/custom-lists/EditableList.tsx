...
export function EditableList({
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        `Είστε σίγουροι ότι θέλετε να διαγράψετε τη λίστα "${list.title}" και όλα τα περιεχόμενά της; Αυτή η ενέργεια δεν μπορεί να αναιρεθεί.`,
      )
    ) {
      return null;
    }
    ...
    try {
      ...
      toast({
        title: "Επιτυχία",
        description: "Η λίστα και όλα τα στοιχεία της διαγράφηκαν.",
      });
      await fetchAllLists();
    } catch (error: any) {
      toast({
