...
export function UnitFloorPlansTab({ unit }: UnitFloorPlansTabProps) {
  ...
  const getFloorsData = useCallback(async () => {
    ...
    const floorsQuery = query(
      ...
      where("__name__", "in", unit.floorIds),
    );
    ...
    if (file && file.type === "application/pdf") {
      ...
      if (file)
        toast({
          ...
          description: "Επιλέξτε αρχείο PDF.",
        });
    }
  };
  ...
  const handleUpload = async () => {
    ...
    const storageRef = ref(
      storage,
      `unit_floor_plans/${unit.id}/${floorId}/${planType}/${file.name}`,
    );
            ...
            <Table>
              ...
              <TableBody>
                {floors.map((floor) => {
                  ...
                  const isUploadingThisFile =
                    isUploading &&
                    selectedFile?.floorId === floor.id &&
                    selectedFile?.planType === id;
                      ...
                      <TableCell>
                        {currentPlanUrl ? (
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {currentPlanUrl.split("%2F").pop()?.split("?")[0]}
                            </a>
                            <Button
                              variant="ghost"
                              size="icon"
                            ...
                            <Button
                              ...
                              onClick={() => handleDelete(floor.id, id)}
                            >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            onChange={(e) => handleFileChange(e, floor.id, id)}
                          />
                          <Button
                            size="sm"
