...
export function UnitsListTable({
  ...
  const isAttachmentList =
    units.length > 0 && units[0].hasOwnProperty("details");
  ...
  if (units.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        {onAddNewUnit && (
          <Button onClick={onAddNewUnit} className="mt-4">
              ...
              >
                {isAttachmentList ? (
                  <>
                    ...
                    <TableCell>
                      {attachment.area ? `${attachment.area} τ.μ.` : "N/A"}
                    </TableCell>
                    <TableCell>
                      {attachment.price ? `€${attachment.price}` : "N/A"}
                    </TableCell>
                    <TableCell>
                      {attachment.sharePercentage
                        ? `${attachment.sharePercentage}%`
                        : "N/A"}
                    </TableCell>
                    <TableCell>
                      {attachment.isStandalone ? "Ναι" : "Όχι"}
                    </TableCell>
                  </>
                ) : (
                  <>
