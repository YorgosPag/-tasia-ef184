...
export function GemiCompanyMetadata({ form }: GemiCompanyMetadataProps) {
  ...
  const autoRegistered = useWatch({
    control: form.control,
    name: "job.autoRegistered",
  });

  const getAutoRegisteredText = () => {
    if (typeof autoRegistered === "boolean") {
      return autoRegistered
        ? "Αυτοεγγεγραμμένη Εταιρεία"
        : "Κανονική Εγγραφή (μέσω Γ.Ε.ΜΗ.)";
    }
    return "-";
  };
    ...
    <div className="grid grid-cols-2 gap-4">
      <FormItem>
        ...
        <FormControl>
          <Badge
            variant="outline"
            className="text-muted-foreground block w-fit mt-2"
          >
            {typeof isBranch === "boolean"
              ? isBranch
                ? "Υποκατάστημα"
                : "Μητρική Εταιρεία"
              : "-"}
        ...
        <FormDescription>
          Η πληροφορία αντλείται αυτόματα από το ΓΕΜΗ.
        </FormDescription>
      </FormItem>
      <FormItem>
