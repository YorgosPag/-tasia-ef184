...
export function AddressCard({ form, index, onRemove }: AddressCardProps) {
    ...
    return (
        <div className="p-4 border rounded-md bg-muted/30 space-y-4 relative">
            <Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1" onClick={onRemove}>
                <Trash2 className="h-4 w-4 text-destructive"/>
            </Button>
            
            <AddressTypeSelect control={form.control} name={`addresses.${index}.type`} />
