...
export function BuildingsSection({ project }: BuildingsSectionProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        setBuildings(
          snapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as Building,
          ),
        );
  ...
  const handleEditBuilding = async (building: Building) => {
    ...
    if (docSnap.exists()) {
      ...
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Could not find building details.",
      });
    }
  };
  ...
  const handleDeleteBuilding = async (buildingId: string) => {
    ...
    try {
      ...
      batch.delete(
        doc(db, "projects", project.id, "buildings", buildingToDelete.id),
      );
      await batch.commit();
      ...
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Δεν ήταν δυνατή η διαγραφή του κτιρίου.",
      });
    }
  };
  ...
  const onSubmitBuilding = async (data: BuildingFormValues) => {
    ...
    try {
      if (editingBuilding) {
        const batch = writeBatch(db);
        batch.update(
          doc(db, "buildings", editingBuilding.topLevelId),
          finalData,
        );
        batch.update(
          doc(db, "projects", project.id, "buildings", editingBuilding.id),
          {
        ...
        const subCollectionRef = doc(
          collection(db, "projects", project.id, "buildings"),
        );
        batch.set(topLevelRef, {
          ...finalData,
          projectId: project.id,
          originalId: subCollectionRef.id,
      ...
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Δεν ήταν δυνατή η υποβολή.",
      });
    } finally {
      ...
      <CardContent>
        {isLoadingBuildings ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Ημ/νία Δημ.</TableHead>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  διαγραφεί οριστικά το κτίριο &quot;
                                  {building.address}&quot; και όλοι οι όροφοι
                                  και τα ακίνητα που περιέχει.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
          ...
          <p className="text-center text-muted-foreground py-8">
            Δεν βρέθηκαν κτίρια για αυτό το έργο.
          </p>
        )}
      </CardContent>
      {isEditor && (
