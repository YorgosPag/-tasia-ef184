    ...
    <div className="space-y-4 py-4">
      {stage.status !== "Ολοκληρώθηκε" && !canCompleteStage && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Το στάδιο δεν μπορεί να ολοκληρωθεί. Παρακαλώ ολοκληρώστε όλα τα
            αντικείμενα του checklist επιθεώρησης.
          </AlertDescription>
        </Alert>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2">
        <DetailItem icon={User} label="Ανάθεση">
          {getCompanyNames(stage.assignedTo, companies)}
        </DetailItem>
        <DetailItem icon={Calendar} label="Έναρξη">
          {formatDate(stage.startDate)}
        </DetailItem>
        <DetailItem icon={CheckCircle} label="Λήξη">
          {formatDate(stage.endDate)}
        </DetailItem>
        <DetailItem icon={Clock} label="Προθεσμία">
          {formatDate(stage.deadline)}
        </DetailItem>
        <DetailItem icon={DollarSign} label="Budget">
          {formatCurrency(stage.budgetedCost)}
        </DetailItem>
        <DetailItem icon={DollarSign} label="Actual">
          {formatCurrency(stage.actualCost)}
        </DetailItem>
        <DetailItem icon={Briefcase} label="Σχετίζεται με">
          {(stage as any).relatedEntityIds?.join(", ") || "-"}
        </DetailItem>
        <DetailItem icon={Link2} label="Εξαρτάται από">
          {stage.dependsOn?.join(", ") || "-"}
        </DetailItem>
        {stage.notes && (
          <div className="col-span-full">
            <DetailItem icon={GripVertical} label="Σημειώσεις">
              {stage.notes}
            </DetailItem>
          </div>
        )}
      ...
      <Checklist
        ...
        onToggle={(index, completed) =>
          onChecklistItemToggle(stage, index, completed, isSubstage)
        }
        onAdd={(task) => onAddChecklistItem(stage, task, isSubstage)}
        onNotesChange={(index, notes) =>
          onInspectionNotesChange(stage, index, notes, isSubstage)
        }
      />
      ...
      {stage.workSubstages && stage.workSubstages.length > 0 && (
        <div className="ml-4 mt-2 border-l-2 pl-4">
          ...
          {stage.workSubstages.map((substage) => (
            <div
              key={substage.id}
              className="mb-4 p-2 rounded-md hover:bg-muted/30"
            >
              <WorkStageItem
      ...
      {!isSubstage && (
        <div className="flex items-center gap-2 mt-4 pt-4 border-t">
          <Button
            variant="outline"
            size="sm"
            onClick={() => onAddWorkSubstage?.(stage.id)}
          >
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Αυτή η ενέργεια θα διαγράψει οριστικά το στάδιο &quot;
                  {stage.name}&quot; και όλα τα υποστάδια του.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Ακύρωση</AlertDialogCancel>
                <AlertDialogAction
                  onClick={() => onDeleteWorkStage(stage)}
      ...
      <div className="mt-6 border-t pt-4">
        ...
        <WorkStageCommentForm
          onSubmit={(comment) => onCommentSubmit(stage, comment, isSubstage)}
        />
      </div>
    </div>
  );
