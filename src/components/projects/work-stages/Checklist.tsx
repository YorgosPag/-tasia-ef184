...
import {
  ...
  formatDate,
} from "@/components/projects/work-stages/utils";
...
export function Checklist({
  ...
  const progress = useMemo(
    () => calculateChecklistProgress(stage.checklist),
    [stage.checklist],
  );
    ...
    <div className="mt-4 space-y-3">
      ...
      <div className="space-y-2">
        {stage.checklist?.map((item, index) => (
          <div
            key={index}
            className="flex flex-col gap-2 bg-muted/50 p-3 rounded-md"
          >
            <div className="flex items-center gap-3">
              ...
              <Label
                ...
                className={`flex-1 flex items-center gap-2 ${item.completed ? "line-through text-muted-foreground" : ""}`}
              >
                {isDocument(item.task) && (
            ...
            {item.completed && item.completedBy && (
              <p className="text-xs text-muted-foreground pl-8">
                Ολοκληρώθηκε από {item.completedBy} στις{" "}
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder="Νέα εργασία επιθεώρησης..."
