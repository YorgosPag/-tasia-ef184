...
import { AddParkingDialog } from './add-parking-dialog';
import { EditParkingDialog } from './edit-parking-dialog';
    ...
    <AlertDialogContent>
      <AlertDialogHeader>
        ...
        <AlertDialogDescription className="pt-4">
          Είστε βέβαιοι ότι θέλετε να διαγράψετε την επιλεγμένη θέση στάθμευσης; Η ενέργεια αυτή δεν μπορεί να αναιρεθεί.
        </AlertDialogDescription>
      </AlertDialogHeader>
      <AlertDialogFooter>
...
export function ParkingToolbar({ 
  ...
  return (
    <div className="p-1.5 border-b flex items-center gap-1 shrink-0">
      {/* Βασικά εργαλεία */}
      <AddParkingDialog trigger={
          <ToolbarButton tooltip="Νέα Θέση" className="text-green-600 hover:text-green-700 dark:text-green-500 dark:hover:text-green-400">
              <Plus className="w-4 h-4" />
            </ToolbarButton>
        } />
      <EditParkingDialog parkingSpot={selectedSpot!} trigger={
          <ToolbarButton tooltip="Επεξεργασία" className="text-blue-600 hover:text-blue-700 dark:text-blue-500 dark:hover:text-blue-400" disabled={!isSpotSelected}>
              <Pencil className="w-4 h-4" />
            </ToolbarButton>
        } />
      <DeleteConfirmationDialog onConfirm={() => console.log('Delete parking spot', selectedSpot?.id)}>
          <ToolbarButton tooltip="Διαγραφή" className="text-red-600 hover:text-red-700 dark:text-red-500 dark:hover:text-red-400" disabled={!isSpotSelected}>
              <Minus className="w-4 h-4" />
            </ToolbarButton>
        </DeleteConfirmationDialog>
      <ToolbarButton tooltip="Προβολή" disabled={!isSpotSelected}><Eye className="w-4 h-4" /></ToolbarButton>
      <ToolbarButton tooltip="Εκτύπωση"><Printer className="w-4 h-4" /></ToolbarButton>

      {/* Διαχωριστής */}
      <div className="w-px h-5 bg-border mx-1" />

      {/* Column Visibility Button με Custom Dropdown */}
