...
import { HelpCircle, Minus, Plus, Save, FileText, Search } from "lucide-react"
    ...
    return (
        <TooltipProvider>
            <div className="p-1.5 border-b flex items-center gap-1">
                ...
                <ToolbarButton tooltip="Επεξεργασία" className="text-blue-600 hover:text-blue-700 dark:text-blue-500 dark:hover:text-blue-400">
                ...
                <ToolbarButton tooltip="Αναζήτηση">
...
export function BuildingsList({ buildings, selectedBuilding, onSelectBuilding }: BuildingsListProps) {
    return (
        <div className="w-[350px] bg-card border rounded-lg flex flex-col shrink-0">
            <div className="p-2 border-b space-y-2">
                <div className="flex items-center gap-2">
                    <FileText className="w-5 h-5" />
                    <h3 className="text-sm font-semibold">Κτίρια</h3>
                </div>
                <div className="space-y-1">
                    <Label htmlFor="company-filter" className="text-xs">Εταιρεία</Label>
                     <Select defaultValue="pagonis-nest">
                        ...
                        <SelectContent>
                            <SelectItem value="pagonis-nest">Ν.Χ.Γ. ΠΑΓΩΝΗΣ & ΣΙΑ Ο.Ε.</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                 <div className="space-y-1">
                    <Label htmlFor="project-filter" className="text-xs">Έργο</Label>
                    <Select defaultValue="palaiologou">
                        <SelectTrigger id="project-filter" className="h-8 text-xs">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="palaiologou">Παλαιολόγου</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        {buildings.map((building) => (
                            <TableRow 
                                key={building.id} 
                                className={cn("cursor-pointer", selectedBuilding.id === building.id ? "bg-primary text-primary-foreground hover:bg-primary/90" : "hover:bg-accent")}
                                onClick={() => onSelectBuilding?.(building)}
                            >
                                <TableCell className="font-medium py-1.5 px-2 text-xs">{building.name}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
