...
const DOCUMENT_RULES: Rule[] = [
    ...
    },
    {
        ...
        pattern: /^[A-Z]{2}\d{7}$/i,
        format: (value: string) => {
            return value.replace(/[^A-Z0-9]/gi, '').toUpperCase().slice(0, 9);
        }
    },
    {
 ...
 */
export function getValidationRule(type?: string): Rule {
    if (!type) {
        return DOCUMENT_RULES[DOCUMENT_RULES.length - 1]; // Default rule
    }
    const typeLower = type.toLowerCase();
    return DOCUMENT_RULES.find(rule => 
        rule.keywords.length > 0 && rule.keywords.every(kw => typeLower.includes(kw))
    ) || DOCUMENT_RULES.find(rule => rule.keywords.length > 0 && rule.keywords.some(kw => typeLower.includes(kw))) || DOCUMENT_RULES[DOCUMENT_RULES.length - 1];
}
