    ...
    if (finalData.documents && Array.isArray(finalData.documents)) {
      ...
      const existingChecklist =
        editingWorkStage && "checklist" in editingWorkStage
          ...
          : [];
      const nonDocumentTasks = existingChecklist.filter(
        (item) => !item.task.startsWith("Doc:"),
      );
...
function useWorkStageChecklist(
  projectId: string,
  workStages: WorkStageWithSubstages[],
