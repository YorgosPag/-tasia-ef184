    ...
    const fetchCalendarData = async () => {
      ...
      try {
        ...
        for (const projectDoc of projectsSnapshot.docs) {
          ...
          const workStagesRef = collection(
            db,
            "projects",
            project.id,
            "workStages",
          ...
          for (const stageDoc of workStagesSnapshot.docs) {
            ...
            for (const subStageDoc of substagesSnapshot.docs) {
              ...
              if (subStage.startDate)
                allEvents.push({
                  id: subStage.id,
                  date: subStage.startDate.toDate(),
                  title: `Έναρξη - ${subStageTitle}`,
                  type: "start",
                  status: subStage.status,
                  projectId: project.id,
                  color: statusColorMap[subStage.status],
                });
              if (subStage.endDate)
                allEvents.push({
                  id: subStage.id,
                  date: subStage.endDate.toDate(),
                  title: `Λήξη - ${subStageTitle}`,
                  type: "end",
                  status: subStage.status,
                  projectId: project.id,
                  color: statusColorMap[subStage.status],
              ...
              if (subStage.deadline)
                allEvents.push({
                  id: subStage.id,
                  date: subStage.deadline.toDate(),
                  title: `Προθεσμία - ${subStageTitle}`,
                  type: "deadline",
                  status: subStage.status,
                  projectId: project.id,
                  color: statusColorMap[subStage.status],
