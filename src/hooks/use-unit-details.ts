...
import {
  newUnitSchema as unitSchema,
  getUnitDataFromForm,
  getAttachmentDataFromForm,
  getStatusClass,
...
import {
  ...
  attachmentSchema,
} from "@/components/units/AttachmentDialog";
...
export function useUnitDetails() {
  ...
  const [editingAttachment, setEditingAttachment] =
    useState<AttachmentFormValues | null>(null);
  ...
  const attachmentForm = useForm<AttachmentFormValues>({
    ...
    defaultValues: { type: "parking", isBundle: true, isStandalone: false },
  });
  ...
  useEffect(() => {
      ...
      (unitDocSnap) => {
        if (!unitDocSnap.exists()) {
          toast({
            variant: "destructive",
            title: "Σφάλμα",
            description: "Το ακίνητο δεν βρέθηκε.",
          });
          router.push("/units");
          return;
        }
        ...
        unitForm.reset({
          ...
          architecturalProjectionsArea:
            unitData.architecturalProjectionsArea?.toString() || "",
    ...
    const unsubscribeAttachments = onSnapshot(
      ...
      (snapshot) => {
        setAttachments(
          snapshot.docs.map(
            (doc) => ({ ...doc.data(), id: doc.id }) as AttachmentFormValues,
          ),
        );
      },
  ...
  const onUnitSubmit = useCallback(
    async (data: UnitFormValues) => {
      ...
      try {
        ...
        toast({
          title: "Επιτυχία",
          description: "Οι αλλαγές στο ακίνητο αποθηκεύτηκαν.",
        });
        ...
        toast({
          variant: "destructive",
          title: "Σφάλμα",
          description: "Η αποθήκευση απέτυχε.",
        });
      } finally {
  ...
  const onSubmitAttachment = useCallback(
    async (data: AttachmentFormValues) => {
      if (!unit) return;
      setIsSubmitting(true);
      ...
      Object.keys(finalAttData).forEach(
        (key) => finalAttData[key] === undefined && delete finalAttData[key],
      );
      ...
      try {
        if (editingAttachment?.id) {
          ...
          toast({
            title: "Επιτυχία",
            description: "Το παρακολούθημα ενημερώθηκε.",
          });
          await logActivity("UPDATE_ATTACHMENT", {
            entityId: editingAttachment.id,
            entityType: "attachment",
            changes: finalAttData,
            projectId: unit.projectId,
          });
        } else {
          ...
          toast({
            title: "Επιτυχία",
            description: "Το παρακολούθημα δημιουργήθηκε.",
          });
          await logActivity("CREATE_ATTACHMENT", {
            entityId: newAttRef.id,
            entityType: "attachment",
            details: finalAttData,
            projectId: unit.projectId,
          });
        }
