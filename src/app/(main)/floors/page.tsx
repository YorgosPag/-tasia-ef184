...
export default function FloorsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeFloors = onSnapshot(
      ...
      (snapshot) => {
        setFloors(
          snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Floor),
        );
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="relative">
        ...
        <Input
          type="search"
          placeholder="Αναζήτηση σε όροφο ή κτίριο..."
            ...
            <Table>
              ...
              <TableBody>
                {filteredFloors.map((floor) => (
                  <TableRow
                    key={floor.id}
                    onClick={() => router.push(`/floors/${floor.id}`)}
                    className="cursor-pointer"
                  >
                    <TableCell className="font-medium">{floor.level}</TableCell>
                    <TableCell>
                      {buildingsMap.get(floor.buildingId) || "Άγνωστο Κτίριο"}
                    </TableCell>
