...
export default function NewUnitPage() {
  ...
  const locationState = useUnitLocationState(
  ...
  const handleQuickCreate = (entity: string) => {
    toast({
      title: "Σημείωση",
      description: `Η δημιουργία νέου "${entity}" δεν υποστηρίζεται από αυτή τη φόρμα.`,
    });
  };
  ...
  const handleGenerateId = async () => {
    if (!floorIds || floorIds.length === 0) {
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Παρακαλώ επιλέξτε τουλάχιστον έναν όροφο.",
      });
      return;
    }
    if (!unitType) {
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Παρακαλώ επιλέξτε τύπο ακινήτου.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Επιτυχία",
        description: `Προτεινόμενος κωδικός: ${nextId}`,
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  const onSubmit = async (data: NewUnitFormValues) => {
    ...
    if (!selectedBuilding || !selectedProject || !user) {
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Λείπουν βασικές πληροφορίες (Κτίριο, Έργο, Χρήστης).",
      });
    ...
    if (!buildingInfo || !buildingInfo.originalId) {
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: "Δεν βρέθηκαν οι πληροφορίες του κτιρίου.",
      });
      setIsSubmitting(false);
      return;
    }
    ...
    finalData.companyId = projects.find(
    ...
    try {
      ...
      const floorInfo = await Promise.all(
        data.floorIds.map(async (id) => {
      ...
      if (floorInfo.length > 0 && floorInfo[0]?.originalId) {
        ...
        const subCollectionUnitRef = doc(
          collection(
            db,
            "projects",
            selectedProject,
            "buildings",
            buildingInfo.originalId,
            "floors",
            firstFloorOriginalId,
            "units",
          ),
        );
        batch.set(subCollectionUnitRef, {
          ...finalData,
          topLevelId: topLevelUnitRef.id,
        });
        batch.set(topLevelUnitRef, {
          ...finalData,
          originalId: subCollectionUnitRef.id,
        });
      } else {
        batch.set(topLevelUnitRef, {
          ...finalData,
      ...
      toast({
        title: "Επιτυχία",
        description: `Το ακίνητο "${finalData.name}" δημιουργήθηκε.`,
      });
      ...
      toast({
        variant: "destructive",
        title: "Σφάλμα",
        description: `Η δημιουργία απέτυχε: ${error.message}`,
      });
    } finally {
      setIsSubmitting(false);
    }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Συμπληρώστε τα παρακάτω πεδία για να δημιουργήσετε ένα νέο
              ακίνητο.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Κωδικός Ακινήτου</FormLabel>
