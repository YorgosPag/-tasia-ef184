...
export function ParkingTableHeader({ 
  ...
  // Συνάρτηση για υπολογισμό βέλτιστου πλάτους στήλης
  const calculateOptimalWidth = (columnIndex: number): number => {
    ...
    // Μέτρηση του header text
    const headerWidth = context.measureText(column.label).width + 40;
    maxWidth = Math.max(maxWidth, headerWidth);
    
    // Μέτρηση όλων των values στη στήλη
    parkingSpots.forEach(spot => {
      const value = spot[column.key];
      let displayValue = '';
      
      if (column.format && value !== null && value !== undefined) {
        displayValue = String(column.format(value));
      } else {
        displayValue = String(value || '');
      }
      
      const textWidth = context.measureText(displayValue).width + 40;
  ...
  // Double click handler για auto-resize
  const handleDoubleClick = (columnIndex: number) => {
    const newWidths = [...columnWidths];
    newWidths[columnIndex] = calculateOptimalWidth(columnIndex);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Απλή λογική: ενημέρωση του τρέχοντος column
      newWidths[resizingIndex] = Math.max(100, startWidths[resizingIndex] + diff);
      
      // Αν υπάρχει επόμενο column, προσαρμόζουμε και αυτό
        ...
        {columns.map((header, index) => (
          ...
          >
            ...
            {index < columns.length - 1 && (
              <div
                className="absolute right-0 top-0 h-full w-2 cursor-col-resize hover:bg-primary/30 active:bg-primary/50 z-10 flex items-center justify-center group transition-colors"
                onMouseDown={(e) => handleMouseDown(e, index)}
                onDoubleClick={() => handleDoubleClick(index)}
                style={{
                  backgroundColor: isResizing && resizingIndex === index ? 'rgb(59 130 246 / 0.5)' : 'transparent'
                }}
                title="Σύρε για αλλαγή μεγέθους ή διπλό κλικ για αυτόματη προσαρμογή"
              >
                <div className="w-px h-6 bg-border group-hover:bg-primary transition-colors"></div>
              </div>
            )}
          </div>
        ))}
