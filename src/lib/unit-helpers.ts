...
export function getUnitDataFromForm(data: NewUnitFormValues) {
  const unitData: { [key: string]: any } = {
    ...
    semiOutdoorArea: data.semiOutdoorArea
      ? parseFloat(data.semiOutdoorArea)
      : undefined,
    architecturalProjectionsArea: data.architecturalProjectionsArea
      ? parseFloat(data.architecturalProjectionsArea)
      : undefined,
    balconiesArea: data.balconiesArea
      ? parseFloat(data.balconiesArea)
      : undefined,
...
export function getAttachmentDataFromForm(data: any) {
  const attachmentData: { [key: string]: any } = {
    ...
    sharePercentage: data.sharePercentage
      ? parseFloat(data.sharePercentage)
      : undefined,
