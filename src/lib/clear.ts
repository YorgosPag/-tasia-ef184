...
async function clearCollection(collectionPath: string) {
  ...
  while (snapshot.size > 0) {
    console.log(
      `Found ${snapshot.size} documents in "${collectionPath}". Preparing to delete...`,
    );
    ...
    for (const doc of snapshot.docs) {
      ...
      if (count >= BATCH_LIMIT) {
        await batch.commit();
        console.log(
          `Committed a batch of ${count} deletions from "${collectionPath}".`,
        );
        batch = writeBatch(db);
        count = 0;
      }
    ...
    if (count > 0) {
      await batch.commit();
      console.log(
        `Committed final batch of ${count} deletions from "${collectionPath}".`,
      );
    }
...
export async function clearTasiaData() {
  ...
  for (const projectDoc of projectsSnapshot.docs) {
    const workStagesSnapshot = await getDocs(
      collection(projectDoc.ref, "workStages"),
    );
    for (const workStageDoc of workStagesSnapshot.docs) {
      // Clear any sub-collections of workStages, like workSubstages
      await clearCollection(
        `projects/${projectDoc.id}/workStages/${workStageDoc.id}/workSubstages`,
      );
    }
