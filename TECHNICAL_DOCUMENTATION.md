...
2.  **Δημιουργία Web App**:
    ...
    - ⚡️ **Quick Pitstop:** Βεβαιωθείτε ότι έχετε αντιγράψει _όλα_ τα κλειδιά από το αντικείμενο `firebaseConfig`. Η παράλειψη κάποιου (π.χ., `storageBucket`) μπορεί να προκαλέσει "σιωπηλά" σφάλματα που δεν φαίνονται αμέσως.
...
3. **Δημιουργία OAuth 2.0 Client ID**:
   ...
   - ⚡️ **Quick Pitstop (Η πιο συχνή παγίδα!):** Τα **Authorized redirect URIs** πρέπει να ταιριάζουν _ακριβώς_ με αυτό που περιμένει η εφαρμογή. Ένα `/` παραπάνω ή η έλλειψή του θα προκαλέσει σφάλμα `invalid_client` ή `redirect_uri_mismatch`.
...
### Βήμα 4: Δημιουργία Αρχείου Περιβάλλοντος (`.env.local`)

1.  Στον κεντρικό φάκελο του project, θα βρείτε ένα αρχείο με όνομα `.env.local`.
2.  Ανοίξτε το και συμπληρώστε τις τιμές που αντιγράψατε στα προηγούμενα βήματα.
    ...
    - ⚠️ **ΑΣΦΑΛΕΙΑ:** Ποτέ μην ανεβάζετε το αρχείο `.env.local` σε δημόσιο repository (π.χ., GitHub). Βεβαιωθείτε ότι το αρχείο `.gitignore` περιέχει μια γραμμή `.env.local`.
...
1.  **Εκκίνηση του Development Server**:
    - ⚡️ **Quick Pitstop:** Κάθε φορά που αλλάζετε κάτι στο `.env.local`, πρέπει να κάνετε επανεκκίνηση του server για να εφαρμοστούν οι αλλαγές.
...
- **A:** Το 99% των περιπτώσεων, το πρόβλημα είναι στα **Authorized redirect URIs** (Βήμα 3.3). Ελέγξτε ότι το URI είναι _ακριβώς_ `http://localhost:9003/api/auth/callback/google` χωρίς επιπλέον κενά ή καθέτους.
